...
func main() {
	...
	err = tmpl.Execute(f, PageData{Tasks: append(tasks, reminders...)})
	f.Close()
	check(err)

	f.Close()

	cmd := exec.Command("sh", "-c", "./wkhtmltopdf --encoding utf-8 --margin-top 1mm --margin-bottom 7mm --margin-left 0mm --margin-right 0mm --page-height 210mm --page-width 47mm --grayscale \"temp.html\" \"to_print.pdf\"")
	_, err = cmd.Output()
	check(err)
